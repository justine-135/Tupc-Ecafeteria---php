...
function formatAMPM(date) {//display minutes, seconds, AM and PM
  ...
  var strTime = hours + ':' + minutes + ' ' + ampm + ' - ' + hourly + ':' + minutes + ' ' + ampm;
  return strTime;
}
...
document.getElementById('display-hour').value = formatAMPM(new Date) + ' ';
    ...
    let quantityElement = document.getElementsByClassName('quantity-element');
      for(let i = 0; i < quantityElement.length; i++){
        ...
        inputVal.addEventListener("change",(event) =>{
          ...
          else if(parseInt(quantityInput.value) > parseInt(quantityInput.getAttribute("max"))){
  ...
  //     let initialPrice = inside[i].getElementsByClassName('pricing')[0];
  //     indivPrice = inside[i].getElementsByClassName('quantity-element')[0].parentElement.childNodes[3];
  //   }
  //   console.log(indivPrice);
  //   // indivPrice + (parseFloat(initialPrice.value) * parseFloat(quantity.value));
  // }
      ...
      if(drpDown.value === "breakfast"){// if duplicates, prevent submit
        ...
        for(let i = 0; i < breakfastTbl.rows.length; i++){//check dupplicates
          if(breakfastTbl.rows[i].childNodes[1].innerHTML == foodName.value ){
                document.getElementById('confirmModalLabel').innerHTML = 'This item is already added';
...
function inventoryFunc(){ //DOM for item.html
  ...
  for(let i = 0; i < banBtn.length; i++){
    banBtn[i].addEventListener('click', (e)=>{
      if(banBtn[i].parentNode.parentNode.childNodes[9].textContent == 'SUCCESS'){
        let yesNo = confirm('Mark this order as "CANCELLED"? This cannot be reverted.')
        if(yesNo !== true){
          e.preventDefault();
        }
      }
...
function formatAMPM(date) {//display minutes, seconds, AM and PM
  ...
  clearTable.addEventListener("click", (e) => {// append new table if true
